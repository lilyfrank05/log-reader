...
- **Max File Size**: 500MB for direct `.log`/`.log_1` uploads; zip uploads are capped at 30MB for the zip file itself, but may contain up to 500MB of uncompressed log content
- **Port**: 5001
...
- **File Size Limit**: 500MB maximum for direct `.log`/`.log_1` uploads; zip files are capped at 30MB (transferred bytes) and may decompress to up to 500MB of log content, guarded against zip bombs by that cap
